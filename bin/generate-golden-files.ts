...
program
  ...
  .option(
    '-c, --category <name>',
    'Filter by category (pods, events, version, cluster-info, api-resources, configmaps, secrets, describe, nodes)'
  )
...
const main = async (options: CliOptions) => {
  ...
  if (options.category) {
    if (!isValidCategory(options.category)) {
      ...
      console.error(
        `Valid categories: pods, events, version, cluster-info, api-resources, configmaps, secrets, describe, nodes`
      )
      process.exit(1)
    }
