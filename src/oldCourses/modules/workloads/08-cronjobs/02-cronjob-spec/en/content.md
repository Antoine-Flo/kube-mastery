...
In the previous lesson, you created a CronJob that runs on a schedule and produces Jobs automatically. That is powerful on its own, but real-world scheduling comes with questions that a simple schedule and template cannot answer: _What happens if a Job is still running when the next one is due? How many old Jobs should the cluster keep around?_
