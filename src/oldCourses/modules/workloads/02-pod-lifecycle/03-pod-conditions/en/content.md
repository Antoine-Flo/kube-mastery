...
That is the role of **Pod conditions**. Think of them as a preflight checklist for an aircraft. Before a plane takes off, the crew verifies fuel, instruments, doors, and clearances — each item is either _checked_ or _not checked_. Similarly, Kubernetes maintains a list of conditions for every Pod, and each one is either `True`, `False`, or `Unknown`. Only when all the critical conditions are `True` does the Pod get the green light to receive traffic.
...
The `reason` and `message` fields are your debugging allies — they tell you _why_ a condition is not met, saving you from guesswork.
