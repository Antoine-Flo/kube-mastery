...
const VALID_ACTIONS: Action[] = ['get', 'describe', 'delete', 'apply', 'create', 'logs', 'exec', 'label', 'annotate', 'version', 'cluster-info', 'api-resources', 'scale']
...
const FLAGS_REQUIRING_VALUES = ['n', 'namespace', 'o', 'output', 'l', 'selector', 'tail', 'c', 'container', 'namespaces', 'output-directory', 'replicas']
...
const validateCommandSemantics = (
    ...
    if ((action === 'delete' || action === 'describe' || action === 'logs' || action === 'exec' || action === 'label' || action === 'annotate' || action === 'scale') && !name) {
