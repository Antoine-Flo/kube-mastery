...
const find = <T extends KubernetesResource>(
  ...
  if (!item) {
    return error(
      `Error from server (NotFound): ${kind.toLowerCase()}s "${name}" not found`
    )
  }
...
export const createResourceRepository = <T extends KubernetesResource>(
  kind: string
) => {
  return {
    createEmpty: () => createEmptyCollection<T>(),
    add: (collection: ResourceCollection<T>, resource: T) =>
