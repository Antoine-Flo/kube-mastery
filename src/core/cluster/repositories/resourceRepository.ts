...
const add = <T extends KubernetesResource>(collection: ResourceCollection<T>, resource: T): ResourceCollection<T> => ({
  items: [...collection.items, resource]
})

// Get all resources (optionally filtered by namespace) (internal use only)
const getAll = <T extends KubernetesResource>(collection: ResourceCollection<T>, namespace?: string): T[] => {
  if (!namespace) {
...
const find = <T extends KubernetesResource>(
  ...
  const item = collection.items.find((r) => r.metadata.name === name && r.metadata.namespace === namespace)
...
const remove = <T extends KubernetesResource>(
  ...
  const index = collection.items.findIndex((r) => r.metadata.name === name && r.metadata.namespace === namespace)
...
const update = <T extends KubernetesResource>(
  ...
  const newItems = [...collection.items.slice(0, index), updated, ...collection.items.slice(index + 1)]
...
export const createResourceRepository = <T extends KubernetesResource>(kind: string) => {
  return {
    ...
    getAll: (collection: ResourceCollection<T>, namespace?: string) => getAll(collection, namespace),
    ...
    update: (collection: ResourceCollection<T>, name: string, namespace: string, updateFn: (resource: T) => T) =>
