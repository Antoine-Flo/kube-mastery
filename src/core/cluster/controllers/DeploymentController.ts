...
export class DeploymentController implements Controller {
    ...
    private enqueueOwnerDeployment(rs: ReplicaSet): void {
        ...
        const ownerDeploy = findOwnerByRef(
            rs,
            'Deployment',
    ...
    private handleDeploymentDeleted(deploy: Deployment): void {
        ...
        for (const rs of ownedReplicaSets) {
            this.eventBus.emit(createReplicaSetDeletedEvent(
                rs.metadata.name,
    ...
    private scaleDownReplicaSets(replicaSets: ReplicaSet[]): void {
        for (const rs of replicaSets) {
            ...
            this.eventBus.emit(createReplicaSetUpdatedEvent(
                rs.metadata.name,
                rs.metadata.namespace,
                scaledDownRs,
                rs,
    ...
    private updateDeploymentStatus(deploy: Deployment, namespace: string): void {
        ...
        this.eventBus.emit(createDeploymentUpdatedEvent(
