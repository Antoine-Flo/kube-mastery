...
You have now covered the full admission pipeline: built-in controllers enforce generic cluster policy, enabling and disabling them is a matter of API server flags, and webhooks extend that pipeline with arbitrary custom logic. Mutating webhooks change objects before storage, validating webhooks approve or reject them after mutation.
