...
`-A` (or `--all-namespaces`) is the cluster-wide view. Instead of showing resources in your current namespace, it shows everything across every namespace, with a NAMESPACE column added to the left. Use this when you are looking for something and are not sure which namespace it lives in.

```bash
kubectl get pods -A
```

`-l` lets you filter by label. Labels are key-value pairs attached to resources, and filtering by them is one of the most practical things you can do. For example, if all your frontend pods carry the label `tier=frontend`, you can list just those with:

```bash
kubectl get pods -l tier=frontend
```

`-o yaml` dumps the full resource definition, including both the spec you wrote and the status that Kubernetes has filled in. This is the output format to reach for when you need to understand exactly what a resource looks like under the hood, or when you want to copy a resource's configuration as a starting point for a new manifest.
...
The output is long and text-formatted, not a table. Scroll through it and you will see sections like Containers (with image, ports, resource requests/limits, environment variables, and volume mounts), Conditions (whether the resource is ready, available, etc.), and Volumes.
...
Kubernetes is extensible, and clusters often have many more resource types than the built-in ones you might know. The `kubectl api-resources` command lists every resource type available in the cluster, along with its short name, API group, whether it is namespaced, and its kind.
...
:::warning
When using `-A` (all namespaces), be mindful that in large clusters this can return thousands of resources. In production environments, it is good practice to narrow your query with a label selector (`-l`) or a specific resource type rather than dumping everything at once.
:::
...
Take your time reading the output of `kubectl describe`. The Events section at the bottom often tells you everything you need to know. Comfortable fluency with these commands will make every other part of Kubernetes operations much easier.
