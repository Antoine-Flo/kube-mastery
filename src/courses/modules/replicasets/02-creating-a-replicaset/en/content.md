...
The selector and the template labels must agree, and as a result, the Pods the ReplicaSet creates will always match its own selector.
...
For deeper inspection, `kubectl describe rs web-rs` shows the full status, the events log, and the current Pod template:
