...
Consider an application that holds an exclusive lock on a resource: a single-instance database engine that memory-maps a data file, a legacy message queue that doesn't support multiple instances, or an application with database migrations that are incompatible with the old schema.
